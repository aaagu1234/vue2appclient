...
export default {
    methods: {
       ...mapMutations([Types.SET_CATEGORY]), //// 将 `this.SET_CATEGORY()` 映射为 `this.$store.commit('SET_CATEGORY')`
       ...mapActions([Types.SET_SLIDES]) //// 将 `this.SET_SLIDES()` 映射为 `this.$store.dispatch('SET_SLIDES')`
        // SET_SLIDES(){  上面和下面是一样的效果
        //         this.$store.dispatch('home/SET_SLIDES')
        // }
   },
    async mounted() {
        // 一面一加载就开始获取数据
